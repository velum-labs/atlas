...
Provides a single, robust implementation for extracting table references from
SQL strings.  Used by alma-analysis (lineage extraction) and related modules.

Strategy:
  1. Parse with sqlglot (dialect-aware).
  2. Walk the AST for ``exp.Table`` nodes while filtering CTE aliases,
     system schemas, and SQL keywords.
  3. On ``ParseError``, fall back to lightweight regex extraction so
     incomplete / non-standard SQL from log capture still produces results.
"""
...
def extract_tables_from_sql(
    ...
    *,
    ...
    Uses sqlglot for accurate AST-based extraction with automatic fallback to
    regex when the SQL cannot be parsed (common with log-captured fragments).
    """
