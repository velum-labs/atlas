...
class HookRuntimeConfig:
    ...
    webhook_timeout_seconds: float = field(
        default_factory=lambda: _env_float("ALMA_HOOK_WEBHOOK_TIMEOUT_SECONDS", 10.0)
    )
    max_retries: int = field(default_factory=lambda: _env_int("ALMA_HOOK_MAX_RETRIES", 1))
    backoff_base_seconds: float = field(
        default_factory=lambda: _env_float("ALMA_HOOK_BACKOFF_BASE_SECONDS", 1.0)
    )
